    ...
    if (help_text[help_file] != null) {
        ...
        require("fs").readFile(help_file, "utf8", function (err, data) {
            if (err) {
                ...
                showUsage(help_file, headers, response); // Recursive call, but since data is a string, the recursion
                                                         // will end
            }
        });
    }
...
function getHandler(options, proxy) {
    var corsAnywhere = {
        ...
        corsMaxAge: 0,                  // If set, an Access-Control-Max-Age header with this value (in seconds) will
                                        // be added.
    ...
    return function (req, res) {
        ...
        var customHeaderPrefix = "my_";
        Object.keys(req.headers).forEach(function (header) {
            if (header.startsWith(customHeaderPrefix)) {
                req.headers[header.replace(customHeaderPrefix, "")] = req.headers[header];
